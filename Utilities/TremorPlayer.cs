...
public class TremorPlayer : ModPlayer
{
    ...
    public override void PostUpdateEquips()
    {
        if (zellariumHead)
        {
            ...
            if (Player.dash <= 0 && Player.dashDelay == 0 && !Player.mount.Active)
            {
                ...
                if (flag2)
                {
                    ...
                    {
                        int num23 = Dust.NewDust(new Vector2(Player.position.X, Player.position.Y), Player.width, Player.height, DustID.BlueTorch, 0f, 0f, 100, default, 2f);
        ...
        if (Player.dashDelay < 0)
        {
            for (int l = 0; l < 0; l++)
            {
                ...
                {
                    num14 = Dust.NewDust(new Vector2(Player.position.X, Player.position.Y + Player.height - 4f), Player.width, 8, DustID.BlueTorch, 0f, 0f, 100, default, 1.4f);
                }
                else
                {
                    num14 = Dust.NewDust(new Vector2(Player.position.X, Player.position.Y + (Player.height / 2) - 8f), Player.width, 16, DustID.BlueTorch, 0f, 0f, 100, default, 1.4f);
                }
    ...
    public bool CustomBiomesMatch(Player other)
    {
        ...
        return modOther.ZoneTowerNova == ZoneTowerNova;
    }
